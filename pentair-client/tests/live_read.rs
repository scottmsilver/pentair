...
async fn connect() -> Client {
    let addr = if let Ok(host) = std::env::var("PENTAIR_HOST") {
        ...
        let resp = pentair_client::discovery::discover().await
            .expect("adapter discovery failed — set PENTAIR_HOST");
        format!("{}.{}.{}.{}:{}", resp.ip[0], resp.ip[1], resp.ip[2], resp.ip[3], resp.port)
    };
...
async fn read_version() {
    ...
    assert!(v.version.contains("POOL"), "version should contain 'POOL': {}", v.version);
...
async fn read_status_bodies() {
    ...
    for body in &s.bodies {
        assert!(body.current_temp >= 40 && body.current_temp <= 120,
            "body {} temp {} out of range", body.body_type, body.current_temp);
        assert!(body.set_point >= 40 && body.set_point <= 104,
            "body {} setpoint {} out of range", body.body_type, body.set_point);
    }
    eprintln!("[read_status_bodies] pool={}°F spa={}°F air={}°F",
...
async fn read_status_circuits() {
    ...
    eprintln!("[read_status_circuits] {} circuits, all IDs correct", s.circuits.len());
    client.disconnect().await.unwrap();
}
...
async fn read_controller_config() {
    ...
    eprintln!("[read_controller_config] {} circuits, {} colors, flags=0x{:x}",
        c.circuits.len(), c.colors.len(), c.equipment_flags.raw());
    client.disconnect().await.unwrap();
}
...
async fn read_pump_0_vs() {
    ...
    assert!(p.circuits[0].speed > 0, "first preset should have nonzero speed");
    assert!(p.circuits[0].is_rpm, "first preset should be RPM");
    eprintln!("[read_pump_0_vs] type=VS, running={}, watts={}, rpm={}, gpm={}",
...
async fn read_pumps_1_7_empty() {
    ...
    for i in 1..=7 {
        ...
        assert_eq!(p.pump_type, 0, "pump {} should be type 0 (not installed)", i);
    }
...
async fn read_chem_no_intellichem() {
    ...
    eprintln!("[read_chem_no_intellichem] valid={}, all zeros (no IntelliChem)", c.is_valid);
    client.disconnect().await.unwrap();
}
...
async fn read_chlorinator() {
    ...
    eprintln!("[read_chlorinator] installed={}, pool={}%, spa={}%",
...
async fn read_schedules() {
    ...
    assert!(recurring.events.len() > 0, "should have recurring schedules");
    ...
    eprintln!("[read_schedules] {} recurring, {} runonce",
        recurring.events.len(), runonce.events.len());
    client.disconnect().await.unwrap();
}
...
async fn read_weather() {
    ...
    assert!(w.sunrise > 300 && w.sunrise < 600, "sunrise {} not in expected range", w.sunrise);
    assert!(w.sunset > 900 && w.sunset < 1200, "sunset {} not in expected range", w.sunset);
    eprintln!("[read_weather] sunrise={} ({:02}:{:02}), sunset={} ({:02}:{:02})",
        w.sunrise, w.sunrise / 60, w.sunrise % 60,
        w.sunset, w.sunset / 60, w.sunset % 60);
    client.disconnect().await.unwrap();
}
...
async fn read_system_time() {
    ...
    eprintln!("[read_system_time] {}-{:02}-{:02} {:02}:{:02}:{:02}",
